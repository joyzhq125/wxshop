namespace Hidistro.SqlDal.Settings
{
    ...
    public class MenuDao
    {
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("select count(*) from VShop_NavMenu where wid=@wid");
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT * FROM VShop_NavMenu WHERE wid=@wid and ParentMenuId = 0");
            this.database.AddInParameter(sqlStringCommand, "wid", DbType.String, wid);
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
                return ReaderConvert.ReaderToList<MenuInfo>(reader);
        ...
        public bool SaveMenu(MenuInfo menu)
        {
            ...
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("INSERT INTO VShop_NavMenu (ParentMenuId,wid,Name, Type,DisplaySequence,  [Content],ShopMenuPic) VALUES(@ParentMenuId,@wid,@Name, @Type, @DisplaySequence, @Content,@ShopMenuPic)");
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("UPDATE VShop_NavMenu SET ParentMenuId = @ParentMenuId,wid=@wid, Name = @Name, Type = @Type, [Content] = @Content,ShopMenuPic=@ShopMenuPic WHERE MenuId = @MenuId");
            this.database.AddInParameter(sqlStringCommand, "ParentMenuId", DbType.Int32, menu.ParentMenuId);
            this.database.AddInParameter(sqlStringCommand, "wid", DbType.String, menu.wid);
            this.database.AddInParameter(sqlStringCommand, "Name", DbType.String, menu.Name);
            this.database.AddInParameter(sqlStringCommand, "Type", DbType.String, menu.Type);
        ...
        {
            this.GetAllMenusCount(menu.wid);
