namespace Hidistro.SqlDal.Settings
{
    ...
    public class ShippingModeDao
    {
        ...
        public bool CreateShippingTemplate(FreightTemplate freightTemplate)
        {
            this.Error = "";
            bool flag = false;
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("INSERT INTO Hishop_FreightTemplate_Templates(wid,Name,FreeShip,MUnit,HasFree)VALUES(@wid,@Name,@FreeShip,@MUnit,@HasFree)");
            this.database.AddInParameter(sqlStringCommand, "wid", DbType.String, freightTemplate.wid);
            ...
            using (DbConnection connection = this.database.CreateConnection())
            {
                ...
                try
                {
                    try
                    {
                        ...
                        {
                            DbCommand command2 = this.database.GetSqlStringCommand(" ");
                            this.database.AddInParameter(command2, "TemplateId", DbType.Int32, result);
                            if ((freightTemplate.FreeShippings != null) && (freightTemplate.FreeShippings.Count > 0))
                            {
                                ...
                                foreach (FreeShipping shipping in freightTemplate.FreeShippings)
                                {
                                    ...
                                    this.database.AddInParameter(command2, "ModeId" + num2, DbType.Int32, shipping.ModeId);
                                    this.database.AddInParameter(command2, "ConditionNumber" + num2, DbType.String, shipping.ConditionNumber);
                                    this.database.AddInParameter(command2, "ConditionType" + num2, DbType.Int32, shipping.ConditionType);
                                    ...
                                    if (shipping.FreeShippingRegions != null)
                                    {
                                        foreach (FreeShippingRegion region in shipping.FreeShippingRegions)
                                        {
                                            ...
                                            this.database.AddInParameter(command2, "RegionId" + num3, DbType.Int32, region.RegionId);
                                            num3++;
                                        }
                                    }
                                    num2++;
                                }
                                command2.CommandText = builder.Append("SELECT @ERR;").ToString();
                                if (((int) this.database.ExecuteScalar(command2, transaction)) != 0)
                                {
                            ...
                            {
                                command2 = this.database.GetSqlStringCommand(" ");
                                this.database.AddInParameter(command2, "TemplateId", DbType.Int32, result);
                                ...
                                foreach (SpecifyRegionGroup group in freightTemplate.SpecifyRegionGroups)
                                {
                                    ...
                                    this.database.AddInParameter(command2, "ModeId" + num5, DbType.Int16, group.ModeId);
                                    this.database.AddInParameter(command2, "FristNumber" + num5, DbType.Int32, group.FristNumber);
                                    this.database.AddInParameter(command2, "FristPrice" + num5, DbType.Currency, group.FristPrice);
                                    this.database.AddInParameter(command2, "AddPrice" + num5, DbType.Currency, group.AddPrice);
                                    this.database.AddInParameter(command2, "AddNumber" + num5, DbType.Int32, group.AddNumber);
                                    this.database.AddInParameter(command2, "IsDefault" + num5, DbType.Int16, group.IsDefault);
                                    ...
                                    if (group.SpecifyRegions != null)
                                    {
                                        foreach (SpecifyRegion region2 in group.SpecifyRegions)
                                        {
                                            ...
                                            this.database.AddInParameter(command2, "RegionId" + num6, DbType.Int32, region2.RegionId);
                                            num6++;
                                        }
                                    }
                                    num5++;
                                }
                                command2.CommandText = builder2.Append("SELECT @ERR;").ToString();
                                if (((int) this.database.ExecuteScalar(command2, transaction)) != 0)
                                {
        ...
        public DataTable GetAllFreightItems(string wid)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("select t.Name,t.FreeShip,t.MUnit,t.HasFree,sp.*,s.RegionId from Hishop_FreightTemplate_SpecifyRegionGroups as sp left join Hishop_FreightTemplate_SpecifyRegions as s on sp.GroupId=s.GroupId  left join Hishop_FreightTemplate_Templates as t on t.TemplateId=sp.TemplateId and t.wid=@wid");
            ...
            using (DbConnection connection = this.database.CreateConnection())
            {
                ...
                try
                {
                    try
                    {
                        ...
                        if (flag)
                        {
                            this.database.AddInParameter(command2, "TemplateId", DbType.Int32, freightTemplate.TemplateId);
                            ...
                            this.database.ExecuteNonQuery(command2, transaction);
                        }
                        if (flag && !freightTemplate.FreeShip)
                        {
                            if ((freightTemplate.FreeShippings != null) && (freightTemplate.FreeShippings.Count > 0))
                            {
                                ...
                                foreach (FreeShipping shipping in freightTemplate.FreeShippings)
                                {
                                    ...
                                    this.database.AddInParameter(command2, "ModeId" + num, DbType.Int32, shipping.ModeId);
                                    this.database.AddInParameter(command2, "ConditionNumber" + num, DbType.String, shipping.ConditionNumber);
                                    this.database.AddInParameter(command2, "ConditionType" + num, DbType.Int32, shipping.ConditionType);
                                    ...
                                    if (shipping.FreeShippingRegions != null)
                                    {
                                        foreach (FreeShippingRegion region in shipping.FreeShippingRegions)
                                        {
                                            ...
                                            this.database.AddInParameter(command2, "RegionId" + num2, DbType.Int32, region.RegionId);
                                            num2++;
                                        }
                                    }
                                    num++;
                            ...
                            if ((flag && (freightTemplate.SpecifyRegionGroups != null)) && (freightTemplate.SpecifyRegionGroups.Count > 0))
                            {
                                ...
                                foreach (SpecifyRegionGroup group in freightTemplate.SpecifyRegionGroups)
                                {
                                    ...
                                    this.database.AddInParameter(command2, "ModeId" + num4, DbType.Int16, group.ModeId);
                                    this.database.AddInParameter(command2, "FristNumber" + num4, DbType.Int32, group.FristNumber);
                                    this.database.AddInParameter(command2, "FristPrice" + num4, DbType.Currency, group.FristPrice);
                                    this.database.AddInParameter(command2, "AddPrice" + num4, DbType.Currency, group.AddPrice);
                                    this.database.AddInParameter(command2, "AddNumber" + num4, DbType.Int32, group.AddNumber);
                                    this.database.AddInParameter(command2, "IsDefault" + num4, DbType.Int16, group.IsDefault);
                                    builder2.Append("Set @GroupId =@@identity;");
                                    if (group.SpecifyRegions != null)
                                    {
                                        foreach (SpecifyRegion region2 in group.SpecifyRegions)
                                        {
                                            ...
                                            this.database.AddInParameter(command2, "RegionId" + num5, DbType.Int32, region2.RegionId);
                                            num5++;
                                        }
                                    }
                                    num4++;
