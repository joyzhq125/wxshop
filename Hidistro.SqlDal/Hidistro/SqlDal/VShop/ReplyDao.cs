namespace Hidistro.SqlDal.VShop
{
    ...
    public class ReplyDao
    {
        ...
        public int GetNoMatchReplyID(int compareid,string wid)
        {
            ...
            string str2 = "select top 1 ReplyId from vshop_Reply where wid=@wid and ReplyType=@ReplyType " + str + "  order by ReplyId desc";
        ...
        public int GetSubscribeID(int compareid,string wid)
        {
            string str = string.Empty;
            if (compareid > 0)
            {
        ...
        public bool HasReplyKey(string key,string wid)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT COUNT(*) FROM vshop_Reply WHERE Keys = @Keys and wid=@wid");
        ...
        public bool HasReplyKey(string key, int replyid,string wid)
        {
            string str = string.Empty;
            if (replyid > 0)
            {
            ...
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT COUNT(*) FROM vshop_Reply WHERE wid=@wid and Keys = @Keys " + str);
            this.database.AddInParameter(sqlStringCommand, "Keys", DbType.String, key);
            this.database.AddInParameter(sqlStringCommand, "wid", DbType.String, wid);
            return (Convert.ToInt32(this.database.ExecuteScalar(sqlStringCommand)) > 0);
        }
        ...
        private bool SaveNewsReply(NewsReplyInfo model)
        {
            ...
            builder.Append("Keys,MatchType,ReplyType,MessageType,IsDisable,LastEditDate,LastEditor,Content,Type,ArticleID,wid)");
            ...
            this.database.AddInParameter(sqlStringCommand, "wid", DbType.String, model.wid);
        ...
        private bool SaveTextReply(TextReplyInfo model)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("insert into vshop_Reply(");
            builder.Append("Keys,MatchType,ReplyType,MessageType,IsDisable,LastEditDate,LastEditor,Content,Type,ActivityId,ArticleID,wid)");
            builder.Append(" values (");
            builder.Append("@Keys,@MatchType,@ReplyType,@MessageType,@IsDisable,@LastEditDate,@LastEditor,@Content,@Type,@ActivityId,@ArticleID,@wid)");
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(builder.ToString());
            this.database.AddInParameter(sqlStringCommand, "Keys", DbType.String, model.Keys);
