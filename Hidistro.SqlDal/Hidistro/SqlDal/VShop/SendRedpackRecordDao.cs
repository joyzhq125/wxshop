namespace Hidistro.SqlDal.VShop
{
    ...
    public class SendRedpackRecordDao
    {
        ...
        {
            string str = "select isnull(sum(Amount),0) from vshop_SendRedpackRecord where IsSend=1";
            if (balancedrawrequestid > 0)
            {
                str = str + " and BalanceDrawRequestID=" + balancedrawrequestid;
            }
            else if (userid > 0)
            {
        ...
        public SendRedpackRecordInfo GetSendRedpackRecordByID(string id = null, string sid = null)
        {
            ...
            if (id != null)
            {
                ...
                str = string.Format("select * FROM vshop_SendRedpackRecord WHERE ID={0}", id);
            }
            else
            {
                ...
                str = string.Format("select * FROM vshop_SendRedpackRecord WHERE BalanceDrawRequestID={0}", sid);
            }
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(str);
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
        ...
        {
            bool flag = false;
            string str = "select top 1 ID from vshop_SendRedpackRecord where BalanceDrawRequestID=" + serialid;
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(str);
