namespace Hidistro.SqlDal.VShop
{
    ...
    public class ActivityDao
    {
        ...
        {
            string str = "DELETE FROM vshop_Activity WHERE ActivityId=@ActivityId; DELETE FROM vshop_Reply WHERE ActivityId = @ActivityId AND [ReplyType] = @ReplyType";
        ...
        {
            string str = "SELECT * FROM vshop_Activity WHERE ActivityId=@ActivityId";
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(str);
            this.database.AddInParameter(sqlStringCommand, "ActivityId", DbType.Int32, activityId);
        ...
        public Hidistro.Entities.Promotions.ActivityDetailInfo GetActivityDetailInfo(int Id)
        {
            ...
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
                return ReaderConvert.ReaderToModel<Hidistro.Entities.Promotions.ActivityDetailInfo>(reader);
            }
        }

        public IList<ActivityInfo> GetAllActivity()
        {
            string str = "SELECT *, (SELECT Count(ActivityId) FROM vshop_ActivitySignUp WHERE ActivityId = a.ActivityId) AS CurrentValue FROM vshop_Activity a ORDER BY ActivityId DESC";
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(str);
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
