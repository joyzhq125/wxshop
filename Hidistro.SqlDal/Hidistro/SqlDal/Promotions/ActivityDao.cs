    ...
    public class ActivityDao
    {
        ...
        public int Create(ActivityInfo act, ref string msg)
        {
            ...
            try
            {
                ...
                sqlStringCommand = this.database.GetSqlStringCommand("SELECT ActivitiesId  FROM Hishop_Activities WHERE isAllProduct=1");
                if (Convert.ToInt32(this.database.ExecuteScalar(sqlStringCommand)) >= 1)
                {
                    msg = "全部商品参与活动时一次只能设置一个活动";
                    return 0;
                }
                sqlStringCommand = this.database.GetSqlStringCommand("INSERT INTO [Hishop_Activities] ([ActivitiesName] ,[ActivitiesType],[StartTime],[EndTime],[ActivitiesDescription],[TakeEffect],[Type],[MemberGrades],[attendTime],[attendType],[isAllProduct],[MeetMoney],[ReductionMoney],[MeetType]) VALUES (@ActivitiesName,@ActivitiesType,@StartTime,@EndTime,@ActivitiesDescription,@TakeEffect,@Type,@MemberGrades,@attendTime,@attendType,@isAllProduct,@MeetMoney,@ReductionMoney ,@MeetType); SELECT CAST(scope_identity() AS int);");
                ...
                {
                    string str = "INSERT INTO [Hishop_Activities_Detail]([ActivitiesId],[MeetMoney],[ReductionMoney],[bFreeShipping],[Integral],[CouponId],[MeetNumber])VALUES(@ActivitiesId,@MeetMoney,@ReductionMoney,@bFreeShipping,@Integral,@CouponId ,@MeetNumber)";
        ...
        public bool Delete(int Id)
        {
            using (DbConnection connection = this.database.CreateConnection())
            {
                ...
                DbCommand command2 = this.database.GetSqlStringCommand("DELETE FROM Hishop_Activities_Detail WHERE ActivitiesId = @Id");
                this.database.AddInParameter(command2, "Id", DbType.Int32, Id);
        ...
        {
            string str = "SELECT ROW_NUMBER() OVER(ORDER BY BeginDate) num,*  FROM ((select Id,Name,MemberGrades,BeginDate,EndDate,isnull(ImgUrl,'') as 'ImgUrl',1 as 'ActivityType' from Hishop_PointExChange_PointExChanges where BeginDate <= getdate() and EndDate >= getdate()) union (select ActivitiesId as 'Id',ActivitiesName as 'Name',MemberGrades,StartTime as 'BeginDate',EndTime as 'EndDate', '' as 'ImgUrl', 2 as 'ActivityType' from Hishop_Activities where StartTime <= getdate() and EndTime >= getdate()) union (select CouponId as 'Id',CouponName as 'Name',MemberGrades,BeginDate,EndDate,isnull(ImgUrl,'') as 'ImgUrl',3 as 'ActivityType' from Hishop_Coupon_Coupons where BeginDate <= getdate() and EndDate >= getdate() and Finished = 0) union (select VoteId as 'Id',VoteName as 'Name','0' as 'MemberGrades',StartDate as 'BeginDate',EndDate,isnull(ImageUrl,'') as 'ImgUrl',4 as 'ActivityType' from Hishop_Votes where StartDate <= getdate() and EndDate >= getdate()) union (select GameId as 'Id',GameTitle as 'Name',ApplyMembers as 'MemberGrades',BeginTime as 'BeginDate',EndTime as 'EndDate', '' as 'ImgUrl', 5 as 'ActivityType' from Hishop_PromotionGame where BeginTime <= getdate() and EndTime >= getdate())) tb ";
            if (types != "0")
            {
                str = str + " where ActivityType in (" + types + ")";
            }
        ...
        {
            string str = "SELECT count(id) FROM ((select Id,1 as 'ActivityType' from Hishop_PointExChange_PointExChanges where BeginDate <= getdate() and EndDate >= getdate()) union (select ActivitiesId as 'Id',2 as 'ActivityType' from Hishop_Activities where StartTime <= getdate() and EndTime >= getdate()) union (select CouponId as 'Id',3 as 'ActivityType' from Hishop_Coupon_Coupons where BeginDate <= getdate() and EndDate >= getdate()) union (select VoteId as 'Id',4 as 'ActivityType' from Hishop_Votes where StartDate <= getdate() and EndDate >= getdate()) union (select GameId as 'Id', 5 as 'ActivityType' from Hishop_PromotionGame where BeginTime <= getdate() and EndTime >= getdate())) tb ";
            if (types != "0")
            {
                str = str + " where ActivityType in (" + types + ")";
            }
        ...
        {
            string str = "select ActivitiesId from Hishop_Activities_Detail where id=" + Activities_DetailID;
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(str);
        ...
        public bool Update(ActivityInfo act, ref string msg)
        {
            ...
            try
            {
                ...
                sqlStringCommand = this.database.GetSqlStringCommand("UPDATE [dbo].[Hishop_Activities]SET [ActivitiesName] = @ActivitiesName,  [ActivitiesType] = @ActivitiesType  ,[StartTime] = @StartTime   ,[EndTime] = @EndTime  ,[ActivitiesDescription] = @ActivitiesDescription ,[Type] = @Type ,[MemberGrades] = @MemberGrades  ,[attendTime] = @attendTime ,[attendType] = @attendType  ,[isAllProduct] = @isAllProduct  ,[MeetMoney] = @MeetMoney  ,[MeetType] = @MeetType  ,[ReductionMoney] = @ReductionMoney where ActivitiesId=@ID");
                this.database.AddInParameter(sqlStringCommand, "ActivitiesName", DbType.String, act.ActivitiesName);
                this.database.AddInParameter(sqlStringCommand, "ActivitiesType", DbType.Int32, act.ActivitiesType);
                this.database.AddInParameter(sqlStringCommand, "ActivitiesDescription", DbType.String, act.ActivitiesDescription);
                this.database.AddInParameter(sqlStringCommand, "Type", DbType.Int32, act.Type);
                ...
                if ((act.Details != null) && (act.Details.Count > 0))
                {
                    string str = string.Format("delete from Hishop_Activities_Detail where ActivitiesId={0}", act.ActivitiesId);
                    sqlStringCommand = this.database.GetSqlStringCommand(str);
                    this.database.ExecuteScalar(sqlStringCommand);
