namespace Hidistro.SqlDal.Promotions
{
    ...
    public class GroupBuyDao
    {
        ...
        public GroupBuyInfo GetGroupBuy(int groupBuyId, DbTransaction trans = null)
        {
            ...
            if (trans != null)
            {
                using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand, trans))
                {
                    if (reader.Read())
                    {
            ...
            using (IDataReader reader2 = this.database.ExecuteReader(sqlStringCommand))
            {
                if (reader2.Read())
                {
                    info = DataMapper.PopulateGroupBuy(reader2);
                }
            }
            return info;
        }
        ...
        private bool SetGroupBuyFailed(int groupBuyId)
        {
            ...
            using (IDataReader reader2 = this.database.ExecuteReader(sqlStringCommand))
            {
                while (reader2.Read())
                {
                    ...
                    string str2;
                    string key = reader2.GetString(0);
                    if (!source.ContainsKey(key))
                    {
                        source[key] = 0;
                    }
                    (dictionary2 = source)[str2 = key] = dictionary2[str2] + reader2.GetInt32(1);
                }
            }
            builder.Clear();
