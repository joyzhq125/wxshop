    ...
    public class PointExChangeDao
    {
        ...
        public bool AddProducts(int exchangeId, string productIds, string pNumbers, string points, string eachNumbers)
        {
            try
            {
                ...
                for (int i = 0; i < strArray.Length; i++)
                {
                    ...
                    {
                        DbCommand command2 = this.database.GetSqlStringCommand("INSERT INTO [Hishop_PointExChange_Products]([exChangeId],[ProductId],[status],[ProductNumber],[PointNumber],[ChangedNumber],eachMaxNumber) VALUES (@exChangeId,@ProductId,@status,@ProductNumber,@PointNumber,@ChangedNumber,@eachMaxNumber) update Hishop_PointExChange_PointExChanges set ProductNumber=ProductNumber+1 where Id=@exChangeId ");
                        this.database.AddInParameter(command2, "exChangeId", DbType.Int32, exchangeId);
                        this.database.AddInParameter(command2, "ProductId", DbType.Int32, int.Parse(strArray[i]));
                        this.database.AddInParameter(command2, "status", DbType.Int32, 0);
                        this.database.AddInParameter(command2, "ChangedNumber", DbType.Int32, 0);
                        this.database.AddInParameter(command2, "ProductNumber", DbType.Int32, int.Parse(strArray2[i]));
                        this.database.AddInParameter(command2, "PointNumber", DbType.Int32, int.Parse(strArray3[i]));
                        this.database.AddInParameter(command2, "EachMaxNumber", DbType.Int32, int.Parse(strArray4[i]));
                        num += this.database.ExecuteNonQuery(command2);
                    }
                }
                return (num > 0);
            }
        ...
        public int Create(PointExChangeInfo exchange, ref string msg)
        {
            ...
            try
            {
                ...
                sqlStringCommand = this.database.GetSqlStringCommand("INSERT INTO [Hishop_PointExChange_PointExChanges]([Name] ,[MemberGrades],[BeginDate],[EndDate],[ProductNumber],[ImgUrl]) VALUES(@Name  ,@MemberGrades  ,@BeginDate  ,@EndDate  ,@ProductNumber,@ImgUrl); SELECT CAST(scope_identity() AS int);");
                this.database.AddInParameter(sqlStringCommand, "Name", DbType.String, exchange.Name);
        ...
        public bool DeleteProduct(int exchangeId, int productId)
        {
            using (DbConnection connection = this.database.CreateConnection())
            {
                ...
                DbCommand command2 = this.database.GetSqlStringCommand("update Hishop_PointExChange_PointExChanges set ProductNumber=ProductNumber-1 where Id=@Id");
                this.database.AddInParameter(command2, "Id", DbType.Int32, exchangeId);
                try
                {
        ...
        public bool DeleteProducts(int actId, string ProductIds)
        {
            using (DbConnection connection = this.database.CreateConnection())
            {
                ...
                DbCommand command2 = this.database.GetSqlStringCommand("update Hishop_PointExChange_PointExChanges set ProductNumber=ProductNumber-1 where Id=@Id");
                this.database.AddInParameter(command2, "Id", DbType.Int32, actId);
                try
                {
                    this.database.ExecuteNonQuery(sqlStringCommand, transaction);
                    this.database.ExecuteNonQuery(command2, transaction);
        ...
        public DataTable GetProducts(int exchangeId, int pageNumber, int maxNum, out int total, string sort, bool order = false)
        {
            ...
            string selectFields = "ProductName,exChangeId,ProductId,status,ProductNumber,PointNumber,ChangedNumber,eachMaxNumber,ThumbnailUrl100";
        ...
        public bool InsertProduct(PointExchangeProductInfo product)
        {
            try
            {
                ...
                {
                    DbCommand command2 = this.database.GetSqlStringCommand("INSERT INTO [Hishop_PointExChange_Products]([exChangeId],[ProductId],[status],[ProductNumber],[PointNumber],[ChangedNumber],eachMaxNumber) VALUES (@exChangeId,@ProductId,@status,@ProductNumber,@PointNumber,@ChangedNumber,@eachMaxNumber) update Hishop_PointExChange_PointExChanges set ProductNumber=ProductNumber+1 where Id=@exChangeId ");
                    this.database.AddInParameter(command2, "exChangeId", DbType.Int32, product.exChangeId);
                    this.database.AddInParameter(command2, "ProductId", DbType.Int32, product.ProductId);
                    this.database.AddInParameter(command2, "status", DbType.Int32, product.status);
                    this.database.AddInParameter(command2, "ProductNumber", DbType.Int32, product.ProductNumber);
                    this.database.AddInParameter(command2, "PointNumber", DbType.Int32, product.PointNumber);
                    this.database.AddInParameter(command2, "ChangedNumber", DbType.Int32, product.ChangedNumber);
                    this.database.AddInParameter(command2, "EachMaxNumber", DbType.Int32, product.EachMaxNumber);
                    flag = this.database.ExecuteNonQuery(command2) > 0;
                }
                return flag;
            }
            catch (Exception)
        ...
        public DataTable Query(ExChangeSearch search, ref int total)
        {
            ...
            try
            {
                ...
                string str2 = "select count(id) as total from Hishop_PointExChange_PointExChanges where " + builder.ToString();
                ...
                string str9 = string.Format("select * from ( select top {0} * from ( {1} ) as t1 {2} ) as t2 {3} ", new object[] { pageSize, str8 + builder.ToString() + str5, str7, str6 });
        ...
        public DataTable QueryExChanged(ExChangedProductSearch search)
        {
            ...
            try
            {
                ...
                string str = "select count(ProductId) as total from Hishop_PointExchange_Changed where " + builder.ToString();
        ...
        public bool Update(PointExChangeInfo exchange, ref string msg)
        {
            ...
            try
            {
                ...
                sqlStringCommand = this.database.GetSqlStringCommand("UPDATE [Hishop_PointExChange_PointExChanges] SET [Name] = @Name ,[MemberGrades] = @MemberGrades ,[BeginDate] = @BeginDate ,[EndDate] = @EndDate ,[ProductNumber] = @ProductNumber , [ImgUrl]=@ImgUrl where Id=@ID");
                this.database.AddInParameter(sqlStringCommand, "Name", DbType.String, exchange.Name);
                this.database.AddInParameter(sqlStringCommand, "MemberGrades", DbType.String, exchange.MemberGrades);
                this.database.AddInParameter(sqlStringCommand, "BeginDate", DbType.DateTime, exchange.BeginDate);
                this.database.AddInParameter(sqlStringCommand, "EndDate", DbType.DateTime, exchange.EndDate);
