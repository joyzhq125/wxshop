namespace Hidistro.SqlDal.Promotions
{
    ...
    public class VoteDao
    {
        ...
        public long CreateVote(VoteInfo vote)
        {
            try
            {
                string str = "INSERT INTO [Hishop_Votes]([VoteName],[IsBackup] ,[MaxCheck],[ImageUrl],[StartDate],[EndDate],[Description],[MemberGrades],[IsMultiCheck])VALUES (@VoteName,@IsBackup,@MaxCheck,@ImageUrl,@StartDate,@EndDate,@Description,@MemberGrades,@IsMultiCheck); SELECT CAST(scope_identity() AS int);";
        ...
        public bool CreateVoteItem(VoteItemInfo item)
        {
            try
            {
                string str = string.Format("INSERT INTO [Hishop_VoteItems] ([VoteId],[VoteItemName],[ItemCount]) VALUES ({0},'{1}',{2})", item.VoteId, item.VoteItemName, item.ItemCount);
                DbCommand sqlStringCommand = this.database.GetSqlStringCommand(str);
        ...
        public VoteInfo GetVoteById(long voteId)
        {
            ...
            if (info != null)
            {
                string str = "select * from Hishop_VoteItems where VoteId=@VoteId";
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT *, (SELECT ISNULL(SUM(ItemCount),0) FROM Hishop_VoteItems WHERE VoteId = v.VoteId) AS VoteCounts FROM Hishop_Votes v");
        ...
        public bool UpdateVote(VoteInfo vote)
        {
            ...
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("UPDATE [Hishop_Votes] SET [VoteName] = @VoteName,[MaxCheck] = @MaxCheck,[ImageUrl] = @ImageUrl ,[StartDate] = @StartDate,[EndDate] = @EndDate,[Description] = @Description,[MemberGrades] = @MemberGrades,[IsMultiCheck] = @IsMultiCheck WHERE VoteId = @VoteId ;");
            this.database.AddInParameter(sqlStringCommand, "VoteName", DbType.String, vote.VoteName);
            this.database.AddInParameter(sqlStringCommand, "MaxCheck", DbType.Int32, vote.MaxCheck);
            this.database.AddInParameter(sqlStringCommand, "ImageUrl", DbType.String, vote.ImageUrl);
            this.database.AddInParameter(sqlStringCommand, "StartDate", DbType.DateTime, vote.StartDate);
        ...
        public bool UpdateVoteItem(VoteItemInfo item)
        {
            try
            {
                string str = "UPDATE [Hishop_VoteItems] SET [VoteItemName] = @VoteItemName where VoteItemId=@VoteItemId";
                DbCommand sqlStringCommand = this.database.GetSqlStringCommand(str);
