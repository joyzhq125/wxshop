namespace Hidistro.SqlDal.Commodities
{
    ...
    public class ProductTypeDao
    {
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("INSERT INTO Hishop_ProductTypes(wid,TypeName, Remark) VALUES (@wid,@TypeName, @Remark); SELECT @@IDENTITY");
            this.database.AddInParameter(sqlStringCommand, "wid", DbType.String, productType.wid);
        ...
        public IList<ProductTypeInfo> GetProductTypes(string wid)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT * FROM Hishop_ProductTypes where wid=@wid");
        ...
        {
            return DataHelper.PagingByTopsort(query.PageIndex, query.PageSize, query.SortBy, query.SortOrder, query.IsCount, "Hishop_ProductTypes", "TypeId", string.IsNullOrEmpty(query.wid) ? string.Empty : string.Format("TypeName LIKE '%{0}%' and wid='{1}'", DataHelper.CleanSearchString(query.TypeName),query.wid), "*");
        }

        public int GetTypeId(string typeName,string wid)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT TypeId FROM Hishop_ProductTypes where TypeName = @TypeName and wid=@wid");
