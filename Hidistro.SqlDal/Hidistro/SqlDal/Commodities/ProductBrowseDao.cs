namespace Hidistro.SqlDal.Commodities
{
    ...
    public class ProductBrowseDao
    {
        ...
        public DataTable GetProducts(MemberInfo member, int? topicId, int? categoryId, int distributorId, string keyWord, int pageNumber, int maxNum, out int toal, string sort, bool isAsc = false, bool isselect = false)
        {
            ...
            builder.Append(" ThumbnailUrl60,ThumbnailUrl100,ThumbnailUrl160,ThumbnailUrl180,ThumbnailUrl220,ThumbnailUrl310,MarketPrice,VistiCounts,wid,");
            if (member != null && member.GradeId != 0)
            {
                discount = new MemberGradeDao().GetMemberGrade(member.GradeId).Discount;
            ...
            builder2.AppendFormat(" AND wid='{0}'", member.wid);
        ...
        public int GetProductsNumber(int distributorId)
        {
            int result = 0;
            StringBuilder builder = new StringBuilder();
            builder.AppendFormat("select count(ProductId) as num from Hishop_DistributorProducts where UserId={0} and ProductId in(select ProductId from vw_Hishop_BrowseProductList where SaleStatus=1)", distributorId);
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(builder.ToString());
