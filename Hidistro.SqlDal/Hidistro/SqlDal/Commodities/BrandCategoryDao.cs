namespace Hidistro.SqlDal.Commodities
{
    ...
    public class BrandCategoryDao
    {
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("DECLARE @DisplaySequence AS INT SELECT @DisplaySequence = (CASE WHEN MAX(DisplaySequence) IS NULL THEN 1 ELSE MAX(DisplaySequence) + 1 END) FROM Hishop_BrandCategories where wid=@wid;INSERT INTO Hishop_BrandCategories(BrandName, Logo, CompanyUrl,RewriteName,MetaKeywords,MetaDescription, Description, DisplaySequence,wid) VALUES(@BrandName, @Logo, @CompanyUrl,@RewriteName,@MetaKeywords,@MetaDescription, @Description, @DisplaySequence,@wid); SELECT @@IDENTITY");
            ...
            this.database.AddInParameter(sqlStringCommand, "wid", DbType.String, brandCategory.wid);
        ...
        public DataTable GetBrandCategories(string wid)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT * FROM Hishop_BrandCategories where wid=@wid ORDER BY DisplaySequence");
        ...
        public DataTable GetBrandCategories(string brandName,string wid)
        {
            string str = "1=1";
