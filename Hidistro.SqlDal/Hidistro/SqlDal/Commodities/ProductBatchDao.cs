namespace Hidistro.SqlDal.Commodities
{
    ...
    public class ProductBatchDao
    {
        ...
        {
            string str = this.getProductBaseInfoSelectSQL(productIds);
        ...
        public DataTable GetSkuMemberPrices(string productIds,string wid)
        {
            StringBuilder builder = new StringBuilder();
            ...
            builder.AppendFormat(" SELECT CAST(GradeId AS NVARCHAR) + '_' + [Name] AS MemberGradeName,Discount FROM aspnet_MemberGrades where wid='{0}'",wid);
        ...
        public bool UpdateVisitCounts(int productId)
        {
            ...
            DbCommand command2 = this.database.GetSqlStringCommand(string.Format("UPDATE Hishop_Products SET VistiCounts = VistiCounts + 1 WHERE ProductId = {0}", productId));
            return (this.database.ExecuteNonQuery(command2) > 0);
        }
    }
}
