namespace Hidistro.SqlDal.Members
{
    ...
    public class MemberGradeDao
    {
        ...
        public bool CreateMemberGrade(MemberGradeInfo memberGrade)
        {
            ...
            {
                str = str + "UPDATE aspnet_MemberGrades SET IsDefault = 0 where wid=@wid ";
            }
            str = str + " INSERT INTO aspnet_MemberGrades ([Name], Description, Points, IsDefault, Discount,TranVol,TranTimes,wid) VALUES (@Name, @Description, @Points, @IsDefault, @Discount,@TranVol,@TranTimes,@wid)";
        ...
        public int GetDefaultMemberGrade(string wid)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT GradeId FROM aspnet_MemberGrades WHERE IsDefault = 1 and wid=@wid");
        ...
        public IList<MemberGradeInfo> GetMemberGrades(string wid)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT * FROM aspnet_MemberGrades where (wid=@wid or IsSys=1)");
            this.database.AddInParameter(sqlStringCommand, "wid", DbType.String, wid);
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT COUNT(GradeId) as Count FROM aspnet_MemberGrades WHERE ((TranVol=@TranVol and TranVol is not null ) or (TranTimes=@TranTimes and TranTimes is not null)) AND GradeId<>@GradeId and wid=@wid;");
            this.database.AddInParameter(sqlStringCommand, "TranVol", DbType.Double, memberGrade.TranVol);
            this.database.AddInParameter(sqlStringCommand, "TranTimes", DbType.Int32, memberGrade.TranTimes);
        ...
        public bool IsExist(string name,string wid)
        {
            string str = "SELECT COUNT(*) FROM dbo.aspnet_MemberGrades WHERE Name='" + name + "' and wid=@wid";
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(str);
            this.database.AddInParameter(sqlStringCommand, "wid", DbType.String, wid);
            return (Convert.ToInt32(this.database.ExecuteScalar(sqlStringCommand)) > 0);
        }

        public int SelectUserCountGrades(int gid,string wid)
        {
            string str = "SELECT COUNT(*) FROM dbo.aspnet_Members WHERE GradeId=" + gid +"and wid=@wid";
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(str);
            this.database.AddInParameter(sqlStringCommand, "wid", DbType.String, wid);
        ...
        {
            string str = "SELECT ActiveDay FROM  dbo.Hishop_UserGroupSet ";
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(str);
            return Convert.ToInt32(this.database.ExecuteScalar(sqlStringCommand));
        }
        ...
        {
            string str = "UPDATE dbo.Hishop_UserGroupSet SET ActiveDay=" + day;
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(str);
        ...
        public bool UpdateMemberGrade(MemberGradeInfo memberGrade)
        {
            ...
            str = str + "UPDATE aspnet_MemberGrades SET [Name] = @Name,[IsDefault]=@IsDefault, Description = @Description, Points = @Points, Discount = @Discount ,TranVol=@TranVol ,TranTimes=@TranTimes WHERE GradeId = @GradeId;";
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(str);
            this.database.AddInParameter(sqlStringCommand, "Name", DbType.String, memberGrade.Name);
            this.database.AddInParameter(sqlStringCommand, "Description", DbType.String, memberGrade.Description);
            this.database.AddInParameter(sqlStringCommand, "Points", DbType.Int32, memberGrade.Points);
