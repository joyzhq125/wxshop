namespace Hidistro.SqlDal.Comments
{
    ...
    public class ProductConsultationDao
    {
        ...
        public DbQueryResult GetConsultationProducts(ProductConsultationAndReplyQuery consultationQuery)
        {
            ...
            builder.AppendFormat(" and wid='{0}'", consultationQuery.wid);
        ...
        {
            string str = "SELECT * FROM Hishop_ProductConsultations WHERE ConsultationId=@ConsultationId";
        ...
        public int GetProductConsultationsCount(int productId, bool includeUnReplied,int UserId)
        {
            StringBuilder builder = new StringBuilder("SELECT count(1) FROM Hishop_ProductConsultations WHERE ProductId =" + productId + " and UserId=" + UserId);
            if (!includeUnReplied)
            {
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("INSERT INTO Hishop_ProductConsultations(wid,ProductId, UserId, UserName, UserEmail, ConsultationText, ConsultationDate)VALUES(@wid,@ProductId, @UserId, @UserName, @UserEmail, @ConsultationText, @ConsultationDate)");
            this.database.AddInParameter(sqlStringCommand, "wid", DbType.String, productConsultation.wid);
