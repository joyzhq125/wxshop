namespace Hidistro.SqlDal.Comments
{
    ...
    public class ProductReviewDao
    {
        ...
        public int GetProductReviewsCount(int productId,int UserId)
        {
            StringBuilder builder = new StringBuilder("SELECT count(1) FROM Hishop_ProductReviews WHERE ProductId =" + productId+" and UserId="+ UserId);
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("INSERT INTO Hishop_ProductReviews (ProductId, wid,UserId, ReviewText, UserName, UserEmail, ReviewDate) VALUES(@ProductId,@wid, @UserId, @ReviewText, @UserName, @UserEmail, @ReviewDate)");
            this.database.AddInParameter(sqlStringCommand, "ProductId", DbType.Int32, review.ProductId);
            this.database.AddInParameter(sqlStringCommand, "wid", DbType.String, review.wid);
