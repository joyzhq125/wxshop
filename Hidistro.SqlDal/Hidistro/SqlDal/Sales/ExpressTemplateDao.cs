namespace Hidistro.SqlDal.Sales
{
    ...
    public class ExpressTemplateDao
    {
        ...
        public IList<FreightTemplate> GetFreightTemplates(string wid)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT * FROM Hishop_FreightTemplate_Templates where wid=@wid order by TemplateId desc");
        ...
        {
            string str = "UPDATE Hishop_ExpressTemplates SET IsUse = 1 WHERE ExpressId = @ExpressId;";
            str = str + "UPDATE Hishop_ExpressTemplates SET IsDefault = 0 WHERE IsDefault = 1 and ExpressId!=@ExpressId;" + "UPDATE Hishop_ExpressTemplates SET IsDefault = ~IsDefault WHERE ExpressId = @ExpressId;";
