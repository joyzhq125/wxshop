namespace Hidistro.SqlDal.Sales
{
    ...
    public class ShipperDao
    {
        ...
        public IList<ShippersInfo> GetShippers(bool includeDistributor,string wid)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT * FROM Hishop_Shippers ");
            if (!includeDistributor)
            {
                sqlStringCommand.CommandText = sqlStringCommand.CommandText + " WHERE DistributorUserId = 0 and wid=@wid";
            }
            else
            {
                sqlStringCommand.CommandText = sqlStringCommand.CommandText + " WHERE wid=@wid";
            }
