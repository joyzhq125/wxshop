namespace Hidistro.SqlDal.Sales
{
    ...
    public class ShippingModeDao
    {
                        ...
                        if (flag)
                        {
                            ...
                            if ((shippingMode.ModeGroup != null) && (shippingMode.ModeGroup.Count > 0))
                            {
                                ...
                                foreach (ShippingModeGroupInfo info in shippingMode.ModeGroup)
                                {
                                    ...
                                    this.database.AddInParameter(command2, "Price" + num2, DbType.Currency, info.Price);
                                    this.database.AddInParameter(command2, "AddPrice" + num2, DbType.Currency, info.AddPrice);
                                    ...
                                    foreach (ShippingRegionInfo info2 in info.ModeRegions)
                                    {
                                        ...
                                        this.database.AddInParameter(command2, "RegionId" + num3, DbType.Int32, info2.RegionId);
                                        num3++;
                                    }
                                    num2++;
                                }
        ...
        {
            string str = "SELECT * FROM Hishop_ShippingTemplates ";
        ...
        public IList<ShippingModeInfo> GetShippingModes()
        {
            ...
            string str = "SELECT * FROM Hishop_ShippingTypes st INNER JOIN Hishop_ShippingTemplates temp ON st.TemplateId=temp.TemplateId Order By DisplaySequence";
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(str);
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
                while (reader.Read())
            ...
            using (DbConnection connection = this.database.CreateConnection())
            {
                ...
                try
                {
                    try
                    {
                        ...
                        {
                            DbCommand command2 = this.database.GetSqlStringCommand(" ");
                            this.database.AddInParameter(command2, "TemplateId", DbType.Int32, shippingMode.TemplateId);
                            ...
                            if ((shippingMode.ModeGroup != null) && (shippingMode.ModeGroup.Count > 0))
                            {
                                foreach (ShippingModeGroupInfo info in shippingMode.ModeGroup)
                                {
                                    ...
                                    this.database.AddInParameter(command2, "Price" + num, DbType.Currency, info.Price);
                                    this.database.AddInParameter(command2, "AddPrice" + num, DbType.Currency, info.AddPrice);
                                    ...
                                    foreach (ShippingRegionInfo info2 in info.ModeRegions)
                                    {
                                        ...
                                        this.database.AddInParameter(command2, "RegionId" + num2, DbType.Int32, info2.RegionId);
                                        num2++;
                                    }
                                    num++;
                                }
