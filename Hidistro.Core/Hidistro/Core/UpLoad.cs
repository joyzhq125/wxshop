...
namespace Hidistro.Core
{
    public class UpLoad
    {
        ...
        public string fileSaveAs(HttpPostedFile postedFile, bool isThumbnail, bool isWater)
        {
            try
            {
                string fileExt = SFUtils.GetFileExt(postedFile.FileName); //文件扩展名，不含“.”
                ...
                string newFileName = SFUtils.GetRamCode() + "." + fileExt; //随机生成新的文件名
                string newThumbnailFileName = "thumb_" + newFileName; //随机生成缩略图文件名
                string upLoadPath = GetUpLoadPath(); //上传目录相对路径
                string fullUpLoadPath = SFUtils.GetMapPath(upLoadPath); //上传目录的物理路径
        ...
        private string GetUpLoadPath()
        {
            ...
            //        break;
            //}
            string path = "/Storage/data/article";
            return path + "/";
        }
        ...
        private bool CheckFileExt(string _fileExt)
        {
            ...
            string fileextension = "gif,jpg,png,bmp,rar,zip,doc,xls,txt,xlsx,docx,jpeg,mp3";
            //检查合法文件
