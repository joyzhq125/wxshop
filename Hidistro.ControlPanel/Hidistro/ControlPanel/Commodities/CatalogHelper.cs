namespace Hidistro.ControlPanel.Commodities
{
    ...
    public sealed class CatalogHelper
    {
        ...
        public static bool DeleteCategory(int categoryId,string wid)
        {
            ManagerHelper.CheckPrivilege(Privilege.DeleteProductCategory);
            bool flag = new CategoryDao().DeleteCategory(categoryId,wid);
            if (flag)
            {
        ...
        public static int DisplaceCategory(int oldCategoryId, int newCategory,string wid)
        {
            return new CategoryDao().DisplaceCategory(oldCategoryId, newCategory,wid);
        }
        ...
        public static DataTable GetBrandCategories(string brandName,string wid)
        {
            return new BrandCategoryDao().GetBrandCategories(brandName,wid);
        }
        ...
        private static DataTable GetCategories(string wid)
        {
        ...
        public static IList<CategoryInfo> GetMainCategories(string wid)
        {
        ...
        public static IList<CategoryInfo> GetSequenceCategories(string wid)
        {
            IList<CategoryInfo> categories = new List<CategoryInfo>();
            foreach (CategoryInfo info in GetMainCategories(wid))
            {
                categories.Add(info);
                LoadSubCategorys(info.CategoryId, categories,wid);
            }
            return categories;
        }

        public static IList<CategoryInfo> GetSubCategories(int parentCategoryId,string wid)
        {
            IList<CategoryInfo> list = new List<CategoryInfo>();
            string filterExpression = "ParentCategoryId = " + parentCategoryId.ToString(CultureInfo.InvariantCulture);
            DataRow[] rowArray = GetCategories(wid).Select(filterExpression);
            for (int i = 0; i < rowArray.Length; i++)
            {
                list.Add(DataMapper.ConvertDataRowToProductCategory(rowArray[i]));
            }
        ...
        private static void LoadSubCategorys(int parentCategoryId, IList<CategoryInfo> categories,string wid)
        {
            IList<CategoryInfo> subCategories = GetSubCategories(parentCategoryId,wid);
