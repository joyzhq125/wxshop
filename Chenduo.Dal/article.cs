...
namespace Chenduo.DAL
{
    ...
    public partial class article
    {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (model.fields.Count > 0)
                        {
                            ...
                            strSql2.Append(SFUtils.DelLastComma(strFieldName.ToString()));
        ...
        public Model.article GetModel(int id)
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    StringBuilder strSql2 = new StringBuilder();
                    strSql2.Append("select top 1 " + SFUtils.DelLastComma(sb.ToString()) + " from " + databaseprefix + "article_attribute_value ");
                    strSql2.Append(" where article_id=@article_id ");
                    SqlParameter[] parameters2 = {
					    new SqlParameter("@article_id", SqlDbType.Int,4)};
                    parameters2[0].Value = id;
        ...
        /// 【微帐号】获得查询分页数据
        /// </summary>
        public DataSet GetWCodeList(string wid,int channel_id, int category_id, int pageSize, int pageIndex, string strWhere, string filedOrder, out int recordCount)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM " + databaseprefix + "article");
