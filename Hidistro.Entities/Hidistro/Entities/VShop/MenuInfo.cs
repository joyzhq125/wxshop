        ...
        public virtual string Url
        {
            get
            {
                ...
                switch (this.BindType)
                {
                    ...
                    case Hidistro.Entities.VShop.BindType.Topic:
                        return string.Format("http://{0}/Vshop/Topics.aspx?TopicId={1}&wid={2}", host, this.Content,this.wid);

                    case Hidistro.Entities.VShop.BindType.HomePage:
                        return string.Format("http://{0}/Default.aspx?wid={1}", host,this.wid);

                    case Hidistro.Entities.VShop.BindType.ProductCategory:
                        return string.Format("http://{0}/ProductList.aspx?wid={1}", host,this.wid);

                    case Hidistro.Entities.VShop.BindType.ShoppingCar:
                        return string.Format("http://{0}/Vshop/ShoppingCart.aspx?wid={1}", host,this.wid);

                    case Hidistro.Entities.VShop.BindType.OrderCenter:
                        return string.Format("http://{0}/Vshop/MemberCenter.aspx?wid={1}", host,this.wid);

                    case Hidistro.Entities.VShop.BindType.MemberCard:
                        return string.Format("http://{0}/Vshop/MemberCard.aspx?wid={1}", host, this.wid);

                    case Hidistro.Entities.VShop.BindType.Url:
                        //return this.Content;
                        return string.Format("http://{0}{1}?wid={2}", host, this.Content,this.wid);
                }
                return string.Empty;
            }
        }
