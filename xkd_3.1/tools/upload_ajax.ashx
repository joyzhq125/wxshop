...
<%@ WebHandler Language="C#" Class="MxWeiXinPF.Web.tools.upload_ajax" %>
...
namespace MxWeiXinPF.Web.tools
{
    ...
    public class upload_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        {
            string _delfile = Globals.RequestQueryStr("DelFilePath");
            HttpPostedFile _upfile = context.Request.Files["Filedata"];
            bool _iswater = false; //默认不打水印
            bool _isthumbnail = false; //默认不生成缩略图

            if (Globals.RequestQueryStr("IsWater") == "1")
                _iswater = true;
            if (Globals.RequestQueryStr("IsThumbnail") == "1")
                _isthumbnail = true;
            if (_upfile == null)
            {
        ...
        private void ManagerFile(HttpContext context)
        {
            ...
            //根目录路径，相对路径
            String rootPath = "Storage/data/article/"; //站点目录+上传目录
            //根目录URL，可以指定绝对路径，比如 http://www.yoursite.com/attached/
            String rootUrl = "Storage/data/article/";
            //图片扩展名
            ...
            String dirPath = SFUtils.GetMapPath(rootPath);
