...
{
    public partial class wx_article_page_edit : AdminPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string _action = Globals.RequestQueryStr("action");
        ...
        private bool DoAdd()
        {
            ...
            if (bll.Add(model) > 0)
            {
                ...
                AddAdminLog(DTEnums.ActionEnum.Add.ToString(), "添加微网站单页" + this.channel_name + "内容:" + model.title); //记录日志
                result = true;
            }
            return result;
        }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (albumArr != null)
            {
                ...
                {
                    string[] imgArr = albumArr[i].Split('|');
                    int img_id = SFUtils.StrToInt(imgArr[0], 0);
                    if (imgArr.Length == 3)
                    {
                        if (!string.IsNullOrEmpty(remarkArr[i]))
                        {
                ...
                && attachIdArr.Length > 0 && attachFileNameArr.Length > 0 && attachFilePathArr.Length > 0 && attachFileSizeArr.Length > 0 && attachPointArr.Length > 0)
            {
                ...
                for (int i = 0; i < attachFileNameArr.Length; i++)
                {
                    int attachId = SFUtils.StrToInt(attachIdArr[i], 0);
                    int fileSize = SFUtils.StrToInt(attachFileSizeArr[i], 0);
                    string fileExt = SFUtils.GetFileExt(attachFilePathArr[i]);
                    int _point = SFUtils.StrToInt(attachPointArr[i], 0);
            ...
            if (bll.Update(model))
            {
                ...
                AddAdminLog(DTEnums.ActionEnum.Edit.ToString(), "修改微网站单页" + this.channel_name + "内容:" + model.title); //记录日志
                result = true;
            }
            return result;
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                ChkAdminLevel("articleinfo", DTEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误啦！", "", "Error");
                    return;
