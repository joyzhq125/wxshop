...
namespace MxWeiXinPF.Web.admin.article
{
    public partial class comment_list : AdminPage
    {
        ...
        {
            int _pagesize;
            if (int.TryParse(SFUtils.GetCookie("article_comment_page_size"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            Response.Redirect(SFUtils.CombUrlTxt("comment_list.aspx", "channel_id={0}&keywords={1}&property={2}",
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    SFUtils.WriteCookie("article_comment_page_size", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(SFUtils.CombUrlTxt("comment_list.aspx", "channel_id={0}&keywords={1}&property={2}",
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！",
                SFUtils.CombUrlTxt("comment_list.aspx", "channel_id={0}&keywords={1}&property={2}", this.channel_id.ToString(), this.keywords, this.property), "Success");
        }
    }
}