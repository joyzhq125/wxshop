...
{
    public partial class wx_article_list : AdminPage
    {
        ...
        {
            ChkAdminLevel("articlemgr", DTEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0; //成功数量
            ...
            AddAdminLog(DTEnums.ActionEnum.Edit.ToString(), "删除微信" + this.channel_name + "频道内容成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
            JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！", SFUtils.CombUrlTxt("wx_article_list.aspx", "channel_id={0}&category_id={1}&keywords={2}&property={3}",
                this.channel_id.ToString(), this.category_id.ToString(), this.keywords, this.property), "Success");
        }
    }
}