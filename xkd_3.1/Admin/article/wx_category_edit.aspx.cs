...
{
    public partial class wx_category_edit : AdminPage
    {
        ...
        private bool DoAdd()
        {
            try
            {
                ...
                if (bll.Add(model) > 0)
                {
                    AddAdminLog(DTEnums.ActionEnum.Add.ToString(), "添加" + this.channel_name + "微网站分类:" + model.title); //记录日志
                    return true;
                }
            }
            catch
        ...
        private bool DoEdit(int _id)
        {
            try
            {
                ...
                if (bll.Update(model))
                {
                    AddAdminLog(DTEnums.ActionEnum.Edit.ToString(), "修改" + this.channel_name + "微网站分类分类:" + model.title); //记录日志
                    return true;
                }
            }
            catch
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                ChkAdminLevel("fenlei", DTEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;
