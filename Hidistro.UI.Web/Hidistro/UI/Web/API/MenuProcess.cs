namespace Hidistro.UI.Web.API
{
    ...
    public class MenuProcess : IHttpHandler, IRequiresSessionState
    {
        ...
        public void AddMenus(HttpContext context)
        {
            ...
            menu.wid = wid;
            if (MenuHelper.CanAddMenu(wid,menu.ParentMenuId))
            {
                if (MenuHelper.SaveMenu(menu))
                {
                    s = "{\"status\":\"0\"}";
                }
            }
        ...
        public void GetMenu(HttpContext context)
        {
            ...
            else
            {
                ...
                if (menu != null)
                {
                    ...
                    s = ((((string.Concat(new object[] { obj2, "{\"menuid\": \"", menu.MenuId, "\"," }) + "\"type\": \"" + menu.Type + "\",") + "\"name\": \"" + menu.Name + "\",") + "\"shopmenupic\": \"" + menu.ShopMenuPic + "\",") + "\"content\": \"" + menu.Content + "\"}") + "]";
                }
                s = s + "}";
                context.Response.Write(s);
            }
        ...
        public void GetTopMenus(HttpContext context)
        {
            context.Response.ContentType = "application/json";
            string s = "{";
            IList<MenuInfo> topMenus = MenuHelper.GetTopMenus(wid);
            ...
            else
            {
                ...
                {
                    IList<MenuInfo> menusByParentId = MenuHelper.GetMenusByParentId(wid,info.MenuId);
                    object obj3 = s;
