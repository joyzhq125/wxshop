namespace Hidistro.UI.Web.API
{
    ...
    public class TaobaoProductHandler : IHttpHandler, IRequiresSessionState
    {
        ...
        private void ProcessTaobaoProductDown(HttpContext context)
        {
            ...
            product.TypeId = new int?(ProductTypeHelper.GetTypeId(context.Request.Form["TypeName"],wid));
            int weight = int.Parse(context.Request.Form["Weight"]);
            Dictionary<string, SKUItem> skus = this.GetSkus(product, weight, context);
            ProductActionStatus status = ProductHelper.AddProduct(product, skus, null, null,wid);
