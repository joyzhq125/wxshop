namespace Hidistro.UI.Web.API
{
    ...
    public class wb : IHttpHandler, IRequiresSessionState
    {
        ...
        private static string generateReplyMsg(string data, string type, string senderId, string receiverId)
        {
            JObject obj2 = new JObject();
            obj2.Add("result", 1);
            obj2.Add("sender_id", (JToken) senderId);
            obj2.Add("receiver_id", (JToken) receiverId);
            obj2.Add("type", (JToken) type);
            try
            {
                obj2.Add("data", (JToken) HttpContext.Current.Server.UrlDecode(data));
            }
            catch (Exception)
            {
            }
