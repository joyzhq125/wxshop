namespace Hidistro.UI.Web.Admin
{
    ...
    public class FileManagerJson : AdminPage
    {
        ...
        protected FileManagerJson() : base("", "")
        {
            this.dir = "image";
        }
        ...
        public void FillTableForDb(string cid, string order, Hashtable table)
        {
            ...
            string str3 = string.Format("select * from Hishop_PhotoGallery where CategoryId=0 order by {0}", order);
            if (result > 0)
            {
                str3 = string.Format("select * from Hishop_PhotoGallery where CategoryId={0} order by {1}", result, order);
            }
            DbCommand sqlStringCommand = database.GetSqlStringCommand(str3);
        ...
        public int GetCategoryFile(int iCategryId, out decimal fileSize)
        {
            ...
            fileSize = 0M;
            Database database = DatabaseFactory.CreateDatabase();
            string str = string.Format("select Count(PhotoId) as FileCount,isnull(Sum(FileSize),0) as AllFileSize from Hishop_PhotoGallery", new object[0]);
            if (iCategryId > 0)
            {
                str = string.Format("select Count(PhotoId) as FileCount,isnull(Sum(FileSize),0) as AllFileSize from Hishop_PhotoGallery where CategoryId={0} ", iCategryId.ToString());
            }
