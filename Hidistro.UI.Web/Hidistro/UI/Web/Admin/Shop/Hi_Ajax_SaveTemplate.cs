namespace Hidistro.UI.Web.Admin.Shop
{
    ...
    public class Hi_Ajax_SaveTemplate : IHttpHandler
    {
        ...
        public string GetGoodTag(HttpContext context, JToken data)
        {
            ...
            {
                string str = "";
                foreach (JToken token in data["content"]["goodslist"])
                {
        ...
        {
            string str = "";
            foreach (JToken token in jo["LModules"])
            {
        ...
        {
            string str = "";
            foreach (JToken token in jo["PModules"])
            {
                str = str + this.Base64Decode(token["dom_conitem"].ToString());
            }
            return str;
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                StreamWriter writer = new StreamWriter(context.Server.MapPath("/Templates/vshop/" +wid+"/"+ str2.Trim() + "/data/default.json"), false, Encoding.UTF8);
                ...
                string path = "/Templates/vshop/" +wid+"/"+ str2;
