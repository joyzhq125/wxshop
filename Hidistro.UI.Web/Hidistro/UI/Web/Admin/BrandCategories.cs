namespace Hidistro.UI.Web.Admin
{
    ...
    public class BrandCategories : AdminPage
    {
        ...
        protected BrandCategories() : base("", "")
        {
        }

        private void BindBrandCategories()
        {
            this.grdBrandCategriesList.DataSource = CatalogHelper.GetBrandCategories(wid);
        ...
        protected void btnorder_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < this.grdBrandCategriesList.Rows.Count; i++)
                {
                    ...
                    int result = 0;
                    if (!int.TryParse((this.grdBrandCategriesList.Rows[i].Cells[4].Controls[1] as HtmlInputText).Value, out result))
                    {
                        flag = false;
