namespace Hidistro.UI.Web.Admin.Trade
{
    ...
    public class ManageOrder : AdminPage
    {
        ...
        protected void btnAcceptRefund_Click(object sender, EventArgs e)
        {
            ...
            if (RefundHelper.EnsureRefund(orderInfo.OrderId, userName, this.hidAdminRemark.Value, int.Parse(this.hidRefundType.Value), true))
            {
                try
                {
                    this.myNotifier.updateAction = UpdateAction.OrderUpdate;
                    this.myNotifier.actionDesc = "确认退款";
                ...
                case "GetOrdersStates":
                    {
                        ...
                        int num7 = OrderHelper.GetCountOrderIDByStatus(OrderStatus.BuyerAlreadyPaid, OrderStatus.ApplyForRefund,this.wid) + OrderHelper.GetCountOrderIDByStatus(OrderStatus.SellerAlreadySent, OrderStatus.ApplyForReturns,this.wid);
        ...
        protected void rptSubList_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if ((e.Item.ItemType == ListItemType.Item) || (e.Item.ItemType == ListItemType.AlternatingItem))
            {
                OrderItemStatusLabel label = (OrderItemStatusLabel)e.Item.FindControl("lblOrderItemStatus");
                label.DetailUrl = "OrderDetails.aspx?OrderId=" + DataBinder.Eval(e.Item.DataItem, "OrderID").ToString() + "#returnInfo";
            }
        }
