namespace Hidistro.UI.Web.Admin.Trade
{
    ...
    public class Closed : AdminPage
    {
        ...
        private OrderQuery GetOrderQuery()
        {
            ...
            if (this.stype == 1)
            {
                query.OrderItemsStatus = OrderStatus.Refunded;
            }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (Globals.RequestFormStr("isCallback"))
            {
                ...
                {
                    base.Response.ContentType = "application/json";
                    int length = OrderHelper.GetAllOrderID(this.wid).Select("OrderStatus=" + 4).Length;
                    int countOrderIDByStatus = OrderHelper.GetCountOrderIDByStatus(OrderStatus.Closed, OrderStatus.Refunded,this.wid);
