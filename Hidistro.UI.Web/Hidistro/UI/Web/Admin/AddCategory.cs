namespace Hidistro.UI.Web.Admin
{
    ...
    public class AddCategory : AdminPage
    {
        ...
        private CategoryInfo GetCategory()
        {
            ...
            bool flag = false;
            if ((Convert.ToDecimal(target.FirstCommission) < 1M) || (Convert.ToDecimal(target.FirstCommission) > 100M))
            {
                this.ShowMsg("输入的佣金格式不正确！", false);
                flag = true;
            }
            if ((Convert.ToDecimal(target.SecondCommission) < 1M) || (Convert.ToDecimal(target.SecondCommission) > 100M))
            {
                this.ShowMsg("输入的佣金格式不正确！", false);
                flag = true;
            }
            if ((Convert.ToDecimal(target.ThirdCommission) < 1M) || (Convert.ToDecimal(target.ThirdCommission) > 100M))
            {
                this.ShowMsg("输入的佣金格式不正确！", false);
                flag = true;
            }
