namespace Hidistro.UI.ControlPanel.Utility
{
    ...
    public class PaymentDropDownList : DropDownList
    {
        ...
        public override void DataBind()
        {
            ...
            if (SettingsManager.GetMasterSettings(false,wid).EnableAlipayRequest)
            {
                foreach (PaymentModeInfo info in SalesHelper.GetPaymentModes(wid))
                {
            ...
            if (SettingsManager.GetMasterSettings(false,wid).EnableOffLineRequest)
            {
                base.Items.Add(new ListItem("线下付款", "99"));
            }
            if (SettingsManager.GetMasterSettings(false,wid).EnablePodRequest)
            {
